...
Brief: This file insert bulk load files in order
...
class SQLInsertPipeline:
    ...
    def __init__(self):
        ...
        self._conn = None
        self._delay_between_files = self._args.delay
    ...
    def _parse_arguments(self):
        ...
        parser.add_argument('--bulk-load-only', action='store_true', default=False,
                          help='Load bulk load only (default: False)')
        
        return parser.parse_args()
    
    def _connect_postgres(self) -> Optional[psycopg2.extensions.connection]:
        """Create a robust PostgreSQL connection with retries"""
        for attempt in range(self._args.max_retries):
            try:
                ...
                if attempt == self._args.max_retries - 1:
    ...
    def _find_sql_directory(self) -> Optional[str]:
        """Locate SQL directory with fallback logic"""
        # 1. Check explicitly provided directory
        if os.path.exists(self._args.sql_dir):
            sql_dir = os.path.abspath(self._args.sql_dir)
            if any(os.path.exists(os.path.join(sql_dir, f)) for f in self._STANDARD_SQL_FILES):
                return sql_dir
        ...
        for location in possible_locations:
            ...
            if os.path.exists(norm_path):
                if any(os.path.exists(os.path.join(norm_path, f)) for f in self._STANDARD_SQL_FILES):
                    return norm_path
    ...
    def run(self):
        ...
        # 1. Locate SQL files
        sql_dir = self._find_sql_directory()
        if not sql_dir:
        ...
        try:
            ...
            if self._args.bulk_load_only:
                for sql_file in self._BULK_LOAD_SQL_FILES:
                    all_files.append((sql_file, os.path.join(sql_dir, sql_file)))
            else:
                for sql_file in self._STANDARD_SQL_FILES:
                    all_files.append((sql_file, os.path.join(sql_dir, sql_file)))
                for sql_file in self._BULK_LOAD_SQL_FILES:
                    all_files.append((sql_file, os.path.join(sql_dir, sql_file)))
            ...
            with tqdm(all_files, desc="Processing SQL files") as pbar_files:
                for sql_file_name, file_path in pbar_files:
                    pbar_files.set_postfix(file=sql_file_name + "...")
                    
                    if self._execute_sql_file(file_path):
                        success_count += 1
                    else:
